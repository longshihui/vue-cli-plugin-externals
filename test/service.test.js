...
describe('项目为单页应用时', () => {
    it('配置common字段时，插件正常运行', () => {
        ...
        let realPluginOptions = getPluginOptions(webpackConfig, 'externals-modules');
    ...
    it('配置pages字段无效', () => {
        ...
        expect(webpackConfig.plugins.has('externals-modules')).toBe(true);
        expect(webpackConfig.plugins.has('page1-externals-modules')).toBe(false);
        expect(webpackConfig.plugins.has('page2-externals-modules')).toBe(false);
    });
});
...
describe('项目为多页应用时', () => {
    ...
    it('配置的pages字段，插件正常运行', () => {
        ...
        let page1Options = getPluginOptions(webpackConfig, 'page1-externals-modules').externals[0];
        let page2Options = getPluginOptions(webpackConfig, 'page2-externals-modules').externals[0];
    ...
    it('当页面级别的外部模块和通用模块相同时，通用模块的优先级比页面模块的优先级高', () => {
        ...
        let page1Options = getPluginOptions(webpackConfig,'page1-externals-modules');
        let page2Options = getPluginOptions(webpackConfig,'page2-externals-modules');
        let commonOptions = getPluginOptions(webpackConfig, 'externals-modules');
